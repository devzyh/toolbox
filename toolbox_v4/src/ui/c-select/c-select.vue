      ...
      >
        <div flex-1 truncate>
          <slot name="displayed-value">
            <input
              v-if="searchable && isOpen" ref="searchInputRef" v-model="searchQuery" type="text"
              :placeholder="translate('ui.c-select.search-placeholder')" class="search-input"
            ...
            <span v-else class="placeholder" lh-normal>
              {{ placeholder ?? translate('ui.c-select.select-placeholder') }}
            </span>
          </slot>
        </div>
          ...
          <template v-if="!filteredOptions.length">
            <slot name="empty">
              <div px-4 py-1 opacity-70>
                {{ translate('ui.c-select.no-results') }}
              </div>
            </slot>
          </template>
          <template v-else>
