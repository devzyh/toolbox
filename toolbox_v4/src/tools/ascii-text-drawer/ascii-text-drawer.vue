...
const font = useStorage('dz-tools:ascii-text-drawer:font', 'Standard');
const width = useStorage('dz-tools:ascii-text-drawer:width', 80);
const output = ref('');
...
<template>
  <div class="mw-600">
    ...
    <n-grid cols="4" x-gap="12" w-full>
      <n-gi span="2">
        <c-select
          v-model:value="font"
          label-position="top"
          :label="translate('tools.ascii-text-drawer.font')"
          :options="fonts"
          searchable="true"
        />
      </n-gi>
      <n-gi span="2">
        <n-form-item
          :label="translate('tools.ascii-text-drawer.width')" label-placement="top" label-width="100"
          :show-feedback="false"
        >
    ...
    <div v-if="processing" flex items-center justify-center>
      <n-spin size="medium" />
      <span class="ml-2">{{ translate('tools.ascii-text-drawer.processing') }}</span>
    </div>

    <c-alert v-if="errored" mt-1 text-center type="error">
      {{ translate('tools.ascii-text-drawer.errored') }}
    </c-alert>

    <n-form-item v-if="!processing && !errored" :label="translate('tools.common.output-label')">
      <TextareaCopyable
        :value="output"
        class="mb-1 mt-1"
      />
